---
title: "Word Networks and Topic Modeling of Research on Technology Use for Teaching Mathematics to Students With Disabilities"
date: "`r Sys.Date()`"
output:
...
This webpage contains R codes and figures from Authors (2022). The entire dataset for data analysis is available in the [Open Science Framework](https://osf.io/a7rn3/?view_only=4f0fcc388f93490394c445e6295e3e29) platform. 
...
toks_processed <- raw_tbl_period %>% preprocess.all(text_col = raw_tbl$abstract, verbose = FALSE)
...
theta_td_mean_cor_beta_selected_update_td <- knitr::kable(theta_td_mean_cor_beta_selected_update, col.names = c("Terms", "Topics", "$r$", "$p$")) 

theta_td_mean_cor_beta_selected_update_td
```



### 3. Distribution of topic probability
...
topic_by_prevalence_publication <- theta_td %>%
    ...
    geom_line(show.legend = TRUE, size = 0.3) +
...
year_plot <- effects_year %>%
    ...
    geom_line(color = "#3B3B3B", size = 0.3) +
...
plot_year_doc_counts <- year_doc_counts_all %>%
  ...
  geom_col(fill = "#979797") + 
    theme_classic() +
    theme(
        axis.line = element_line(color = "#3B3B3B", size = 0.2),
        axis.title = element_text(size = 11, color = "#3B3B3B"),
